...
public class TrajectoryHelpers {
	 ...
	 * @param level The wanted CORAL level (L1, L2, L3, L4). TODO might remove L1 as an option
	...
	public static Trajectory generateGamepieceBasedTrajectory(
		...
		if (Util.epsilonEquals(
			...
			for (int i = 0; i < 10; ++i) {
				...
				state.pose = transformedTargetPose;
				state.time = (double) i * 0.05;
	 ...
	 * @param idealApproachDeadband The allowed difference (+-) from the ideal heading that the trajectory is allowed to generate with.
	 * @param level The wanted gamepiece level (L1, L2, L3, L4). TODO might remove L1 as an option
	...
	public static Pose2d getDriveTargetPose(Pose2d endEffectorPose, Angle idealApproachDeadband, Level level) {
		Rotation2d angle = angleToApproach(
				endEffectorPose.getTranslation(),
				endEffectorPose.getRotation(),
				idealApproachDeadband); // gets transformed from End Effector to Drive center when we pass in parameters
	...
	public static Trajectory toWPITrajectoryWithEndingPose(
			AutoTrajectory trajectory, Pose2d coralTargetPose, Level level) {
		Pose2d transformedTargetPose = getDriveTargetPose(coralTargetPose, Units.Degrees.of(0.0), level);

		Trajectory t = new Trajectory();

		for (int i = 0; i < trajectory.getRawTrajectory().samples().size() - 2; ++i) {
			TrajectorySample<?> sample = trajectory.getRawTrajectory().samples().get(i);
			State state = new State();
			state.pose = RobotConstants.isRedAlliance ? sample.flipped().getPose() : sample.getPose();
			state.time = sample.getTimestamp();
			t.getStates().add(state);
		}

		LogUtil.recordPose2d("Auto Target Pose", transformedTargetPose);

		State finalState = new State();
		finalState.pose =
				transformedTargetPose.transformBy(new Transform2d(Translation2d.kZero, Rotation2d.k180deg));
		finalState.time = t.getTotalTime()
				+ 2.0 * trajectory.getRawTrajectory().samples().get(1).getTimestamp();
		t.getStates().add(finalState);
		return t;
	}
