...
public class FollowXLineSwerveRequestCommand extends Command {
	...
	public ChassisSpeeds calculateSpeeds() {
		LinearVelocity ySpeedDesired = DriveConstants.kMaxSpeedTippy.times(DriveConstants.getDeadbandedStick(
				ControlBoardConstants.mDriverController.getLeftX() * -1.0)); // Drive left with negative X (left)
		...
		// SmartDashboard.putNumber("Desired x speed", xSpeedDesired.in(Units.MetersPerSecond));
		SmartDashboard.putNumber("current x", currentX.in(Units.Meters));
		SmartDashboard.putNumber("desired x", desiredX.in(Units.Meters));
		// SmartDashboard.putNumber("Desired y speed", ySpeedDesired.in(Units.MetersPerSecond));
