...
public class FollowTrajectoryCommand extends Command {
	...
	public FollowTrajectoryCommand(
		...
		for (State state : trajectory.getStates()) {
			poseList.add(new Pair<>(state.pose, Units.Seconds.of(state.time)));
		}
	...
	public void initialize() {
		interpolable = new Util.Pose2dTimeInterpolable(
				trajectory, Drive.mInstance.getPose().getRotation(), targetRotation); // HELP
	...
	public boolean atEndPose() {
		...
		// 		"Auto Align Traj/Chihuahua",
		// 		Units.Meters.of(currentPose.getTranslation().getDistance(finalPose.getTranslation()))
		// 				.in(Units.Centimeters));
		// SmartDashboard.putNumber(
		// 		"Auto Align Traj/AngleModulus",
