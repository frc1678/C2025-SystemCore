...
import frc.robot.trajectories.DriveToCoralCommand;
import frc.robot.trajectories.FollowSyncedPIDToPose;
import frc.robot.trajectories.FollowXLineSwerveRequestCommand;
import frc.robot.trajectories.PIDToPoseCommand;
import frc.robot.trajectories.SyncedTagPIDToPose;
...
public class Superstructure extends SubsystemBase {
	...
	public Command tuckAfterProcessor() {
		return Commands.parallel(
						...
						Commands.sequence(
								...
								AlgaeDeploy.mInstance.setpointCommand(AlgaeDeploy.UNJAM_EE),
								MotionPlanner.safePivotAndElevatorToPosition(Pivot.CORAL_INTAKE, Elevator.EE_UNJAM),
								AlgaeDeploy.mInstance.setpointCommandWithWait(AlgaeDeploy.UNJAM_EE),
								MotionPlanner.safePivotAndElevatorToPosition(Pivot.CORAL_INTAKE, Elevator.STOW),
								Commands.parallel(
										AlgaeDeploy.mInstance.setpointCommandWithWait(AlgaeDeploy.STOW),
										CoralDeploy.mInstance.setpointCommandWithWait(CoralDeploy.STOW_CLEAR),
	...
	public Command coralIntakeToEndEffector() {
		return Commands.sequence(
						Commands.parallel(
								...
								EndEffector.mInstance.setpointCommand(
										EndEffector
												.CORAL_FEED), // Start spinning EE immediatly in case one stuck half on
		...
		return Commands.sequence(Commands.defer(
				() -> {
					if (level == Level.L1) {
						...
						return new SyncedTagPIDToPose(finalL1Pose, level, true);
					} else {
						...
						return new SyncedTagPIDToPose(scoringPose, level);
					}
				},
	...
	public Command goToReefIntakePose(Face face) {
		Supplier<Command> supplier = () -> {
			...
			return new SyncedTagPIDToPose(pose, level);
		};
		return Commands.defer(supplier, Set.of(Drive.mInstance));
	}
	...
	public Command autoFindCoral() {
		Set<Subsystem> requirements = Set.of(
				Drive.mInstance,
				CoralDeploy.mInstance,
		...
		Supplier<Command> supplier = () -> new DriveToCoralCommand().deadlineFor(coralIntakeToHold());

		return Commands.defer(supplier, requirements);
	}
