...
public class Pivot extends ServoMotorSubsystem<MotorIOTalonFX> {
	...
	private Angle getCancoderPositionWithoutSimCrashing() {
		try {
			return directCancoder.getPosition().getValue();
		} catch (Exception e) {
			return getPosition();
		}
	}
	...
	private void resetPositionIfNeeded() {
		Angle cancoderAngle = getCancoderPositionWithoutSimCrashing();
		Angle currentReadPosition = getPosition();
		lastShouldReset = !Util.epsilonEquals(
						currentReadPosition.baseUnitMagnitude(),
						cancoderAngle.baseUnitMagnitude(),
						Units.Degrees.of(3.0).baseUnitMagnitude())
				&& getVelocity().abs(Units.DegreesPerSecond) < 30.0;
		if (resetDebouncer.calculate(lastShouldReset)) {
			setCurrentPosition(cancoderAngle);
			SmartDashboard.putNumber(name + "/Reset/Original Degrees", currentReadPosition.in(Units.Degrees));
			SmartDashboard.putNumber(name + "/Reset/New Degrees", cancoderAngle.in(Units.Degrees));
			SmartDashboard.putNumber(name + "/Reset/Timestamp", Timer.getFPGATimestamp());
		}
	}
}
