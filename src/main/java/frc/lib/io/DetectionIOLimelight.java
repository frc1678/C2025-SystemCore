...
public class DetectionIOLimelight extends DetectionIO {
		...
		if (pipelineToSet == LimelightHelpers.getCurrentPipelineIndex(config.name)) {
			if (pipelineToSet == DetectionMode.AUTO.index) {
				...
				for (Coral coral : tracker) {
					// if (i >= maxI) {
					// 	publishers.add(
					// 			visTable.getStructTopic("CoralPose" + i, Pose2d.struct).publish());
					// 	maxI = i;
					// }
					// publishers.get(i).set(coral.coralPose);
					// i++;
	...
	public Pose2d getCoralPose(Translation2d base) {
		...
		return bestCoralPose; // Will reutrn Null if no coral
	}
	...
	public Translation2d calcDistToCoral(double tx, double ty) {
		...
		// Distance distAwayYToGround = edu.wpi.first.units.Units.Meters.of(Math.cos(totalAngleY))
		// 							.div(config.coralDetectionRobotToCameraOffset.getMeasureZ()
		// 								.minus(DetectionConstants.kCoralRadius).in(Meters));
		...
		// SmartDashboard.putNumber(config.name + "/Total Angle X Rads", totalAngleX);
		// SmartDashboard.putNumber(config.name + "/Total Angle Y Rads", totalAngleY);
	...
	public void setLatestEstimate(PoseEstimate poseEstimate, int minTagNum) {
		...
		// SmartDashboard.putNumber(config.name + "/Tag Area", poseEstimate.avgTagArea);
		// SmartDashboard.putNumber(config.name + "/Tag Dist", poseEstimate.avgTagDist);
		// SmartDashboard.putNumber(config.name + "/Timestamp", poseEstimate.timestampSeconds);
		// SmartDashboard.putNumber(config.name + "/Phoenix Timestamp", Utils.getCurrentTimeSeconds());
